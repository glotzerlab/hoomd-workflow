...
pub fn simulate_one(directory: &Path) -> anyhow::Result<()> {
    set_current_dir(directory).with_context(|| format!("error switching to job directory `{}`", directory.display()))?;

    let mut model = get_model().with_context(|| format!("error initializing model in `{}`", directory.display()))?;

    while model.step() < TOTAL_STEPS {
        if model.step().is_multiple_of(1_000) {
            info!("Step {} / {TOTAL_STEPS} ({}%)", model.step(), model.step() as f64 /
            TOTAL_STEPS as f64 * 100.0);
        }
        model.advance()?;
    }
